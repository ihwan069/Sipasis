            ...
            <div class="col-md-4 col-xs-12">
              <div class="form-group">
                <select name="c_kelas" class="form-control select2" required="">
                  ...
                  <?php $opkel = mysqli_query($con, "SELECT * FROM kelas order by kelas asc ");
                  while ($dopkel = mysqli_fetch_array($opkel)) { ?>
...
<?php if (isset($_GET['q']) and isset($_GET['s'])) {
  $kel = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM kelas where c_kelas='$_GET[q]' "));
  $s = str_replace("_", " ", $_GET['s']);
  $k = str_replace("_", " ", $_GET['q']);
?>
  <div class="row">
    <div class="col-xs-12 col-md-12 col-lg-12">
      <div class="box box-info">
        ...
        <form action="<?php echo $basegu; ?>nextinput" method="post">
          <div class="box-body">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                <?php $smk = mysqli_query($con, "SELECT * FROM siswa where c_kelas='$k' and (nisn like '%$s%' or nama like '%$s%') order by nama asc ");
                $vr = 1;
                $jumakh = mysqli_num_rows($smk);
                if ($jumakh > NULL) {
