<?php
$jsi = mysqli_query($con, "SELECT * FROM siswa ");
$hsi = mysqli_num_rows($jsi);
$jgu = mysqli_query($con, "SELECT * FROM guru ");
$hgu = mysqli_num_rows($jgu);
$jpe = mysqli_query($con, "SELECT * FROM pelanggaran ");
$hpe = mysqli_num_rows($jpe);
?>
    ...
    $(document).ready(function() {
      chart = new Highcharts.Chart({
        ...
        series: [
          ...
          while ($dkel = mysqli_fetch_array($kel)) {
            $jum = mysqli_query($con, "SELECT * FROM pelanggaran where c_kelas='$dkel[c_kelas]' ");
            $jumpel = mysqli_num_rows($jum); ?> {
...
<div class="row">
  <?php $pps = mysqli_query($con, "SELECT c_siswa, sum(bobot) as jb from pelanggaran group by c_siswa order by jb desc limit 4 ");
        ...
        <div class="box-body box-profile">
          ...
          <ul class="list-group list-group-unbordered">
            <li class="list-group-item">
              <?php if ($hpps['jb'] > 20) {
                $style = 'style="font-size: 28px;margin-top: -10px;color:#d9534f;"';
              } else {
                $style = 'style="font-size: 28px;margin-top: -10px;"';
              } ?>
              <b>Poin Pelanggaran<a class="pull-right" <?php echo $style . '>' . $hpps['jb']; ?></a></b>
            </li>
          </ul>
          <a href="<?php echo $basead; ?>search/<?php echo urlencode($sis['nisn']); ?>" class="btn btn-primary btn-block"><b>Lihat Pelanggaran</b></a>
        </div>
      </div>
    </div>
  <?php } ?>
    ...
    <div class="box">
      ...
      <div class="box-body">
        <table class="table table-hover">
          ...
          <tbody>
            <?php $vr = 1;
            $pyd = mysqli_query($con, "SELECT c_benpel, count(c_benpel) as bp from pelanggaran group by c_benpel order by bp desc limit 5 ");
            while ($hpyd = mysqli_fetch_array($pyd)) {
