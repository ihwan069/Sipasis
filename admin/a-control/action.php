...
class action
{
  ...
  {
    $ben = mysqli_query($con, "SELECT * FROM benpel where c_katbenpel='$c_katbenpel' ");
    while ($hpel = mysqli_fetch_array($ben)) {
  ...
  {
    $losis = mysqli_query($con, "SELECT * FROM siswa where c_kelas='$c_kelas' ");
    while ($load = mysqli_fetch_array($losis)) {
