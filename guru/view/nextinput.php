      ...
      <div class="box box-primary">
        ...
        <div class="nav-tabs-custom">
          <ul class="nav nav-tabs">
            <?php $smkkat = mysqli_query($con, "SELECT * FROM katbenpel order by katbenpel asc ");
            $vr = 1;
            while ($akhkat = mysqli_fetch_array($smkkat)) { ?>
              <li><a href="#<?php echo $akhkat['c_katbenpel']; ?>" data-toggle="tab"><?php echo $akhkat['katbenpel']; ?></a></li>
            <?php } ?>

            <li> <button href="#selesai" data-toggle="tab" class="btn btn-success">Selesaikan</button></li>

          </ul>
          <div class="tab-content">
            <?php $ngikat = mysqli_query($con, "SELECT * FROM katbenpel order by katbenpel asc ");
            while ($ngi = mysqli_fetch_array($ngikat)) { ?>
              <div class="tab-pane" id="<?php echo $ngi['c_katbenpel']; ?>">
                <table class="table table-bordered table-hover">
                  ...
                  <tbody>
                    <?php $smkben = mysqli_query($con, "SELECT * FROM benpel where c_katbenpel='$ngi[c_katbenpel]' order by bobot asc ");
                    $vr = 1;
                    while ($akhben = mysqli_fetch_array($smkben)) { ?>
                      <tr>
